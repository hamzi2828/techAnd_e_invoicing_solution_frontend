...
export default function CreateCreditNote() {
    ...
    const loadData = async () => {
      try {
        ...
        if (companyToSelect) {
          ...
          const creditNoteNumber = await CreditNoteService.fetchNextCreditNoteNumber(companyId);

          // Load reference invoices for the selected company
          const invoicesResult = await CreditNoteService.getInvoicesForReference(companyId);
          ...
          const initialInvoiceData: InvoiceData = {
            ...
            grandTotal: 0,
            notes: '',
            termsAndConditions: 'This credit note will be applied to your account.',
            paymentTerms: 'Net 30',
        ...
        const fallbackInvoice: InvoiceData = {
          invoiceNumber: `CN-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000000)).padStart(6, '0')}`,
          invoiceType: '' as const,
          zatcaInvoiceTypeCode: '' as const,
          issueDate: new Date().toISOString().split('T')[0],
          dueDate: InvoiceService.getDefaultDueDate(),
  ...
  // Filter customers based on invoice type (B2B/B2C)
  ...
  const loadReferenceInvoices = async (companyId: string) => {
    ...
    try {
      const result = await CreditNoteService.getInvoicesForReference(companyId);
  ...
  const refreshInvoiceNumber = async () => {
    ...
    try {
      const newCreditNoteNumber = await CreditNoteService.fetchNextCreditNoteNumber(companyId);
      setInvoice(prev => prev ? { ...prev, invoiceNumber: newCreditNoteNumber } : prev);
      setMessage({ type: 'success', text: 'Credit note number refreshed' });
      setTimeout(() => setMessage(null), 3000);
    } catch (error) {
      console.error('Error refreshing credit note number:', error);
      setMessage({ type: 'error', text: 'Failed to refresh credit note number' });
    } finally {
  ...
  const handleSave = async () => {
    ...
    if (!selectedReferenceInvoiceId) {
      setMessage({ type: 'error', text: 'Original invoice reference is required for credit notes (ZATCA requirement)' });
      return;
    }
    ...
    setIsLoading(true);
    try {
      // Transform line items to credit note format
      ...
      if (result?.creditNote) {
        setMessage({ type: 'success', text: 'Credit note saved as draft successfully! Redirecting...' });

        // Redirect to credit notes list after 2 seconds
        setTimeout(() => {
      ...
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'Failed to save credit note' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Issue refunds or adjustments to customers</p>
          </div>
        </div>
        <div className="flex space-x-3">
          ...
          <button
            onClick={handleSave}
            disabled={isLoading}
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-600 via-indigo-600 to-primary text-white rounded-lg hover:from-primary hover:via-indigo-600 hover:to-purple-600 transition-all shadow-md hover:shadow-lg disabled:opacity-50"
          >
            <Save className="h-4 w-4" />
            <span className="font-medium">{isLoading ? 'Saving...' : 'Save Credit Note'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
              ...
              <select
                ...
                onChange={async (e) => {
                  ...
                  if (company) {
                    ...
                    try {
                      const newNumber = await CreditNoteService.fetchNextCreditNoteNumber(companyId);
              ...
              {invoice.invoiceType && (
                <p className="mt-1 text-xs text-gray-500">
                  {invoice.invoiceType === 'standard'
                    ? 'Business-to-Business credit note'
                    : 'Business-to-Consumer credit note'}
                </p>
              )}
            </div>
            <div>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                {companyName && (
                                  <div className="text-sm text-purple-600">{companyName}</div>
                                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-purple-50 rounded-lg border border-purple-200 p-6">
            <h3 className="text-lg font-semibold text-purple-900 mb-2">Credit Note</h3>
            <p className="text-sm text-purple-700">
              This credit note will reduce the customer&#39;s outstanding balance. The amount shown represents a credit to be applied.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-purple-50 rounded-lg border border-purple-200 p-6">
            <h3 className="text-lg font-semibold text-purple-900 mb-4">Credit Summary</h3>
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{invoice.currency} {invoice.totalTax.toFixed(2)}</span>
              </div>
              <div className="border-t border-purple-200 pt-3">
                <div className="flex justify-between">
                  <span className="text-lg font-semibold text-purple-900">Total Credit</span>
                  <span className="text-xl font-bold text-purple-600">-{invoice.currency} {invoice.grandTotal.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
