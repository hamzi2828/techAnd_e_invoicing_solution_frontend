...
import { DebitNote, DebitNoteFilters as IDebitNoteFilters, DebitNoteStats as IDebitNoteStats } from './types';
import { DebitNoteService } from './services/debitNoteService';
import { ValidationModal } from './components';
import { ChevronLeft, ChevronRight, Plus, FileText, Eye, Edit, Send, Download, Trash2, Search, RefreshCw, TrendingUp, Clock, CheckCircle, RotateCcw, ShieldCheck, Loader2, X, XCircle, FileArchive } from 'lucide-react';
...
export default function DebitNotesPage() {
  ...
  const [stats, setStats] = useState<IDebitNoteStats>({
    totalDebitNotes: 0,
    draftDebitNotes: 0,
    sentDebitNotes: 0,
    paidDebitNotes: 0,
  ...
  const [filters, setFilters] = useState<IDebitNoteFilters>({
    status: 'all',
    paymentStatus: 'all',
    search: '',
    startDate: '',
  ...
  const [selectedDebitNoteForValidation, setSelectedDebitNoteForValidation] = useState<DebitNote | null>(null);
  ...
  const [debitNotesToDelete, setDebitNotesToDelete] = useState<DebitNote[]>([]);

  // Download confirmation modal state
  const [showDownloadConfirmModal, setShowDownloadConfirmModal] = useState(false);
  const [debitNotesToDownload, setDebitNotesToDownload] = useState<DebitNote[]>([]);
  ...
  const loadDebitNotes = useCallback(async () => {
    ...
    try {
      const result = await DebitNoteService.getAllDebitNotes(filters);
  ...
  const loadStats = async () => {
    ...
    try {
      const statsData = await DebitNoteService.getDebitNoteStats();
      setStats(statsData);
    } catch (error) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedDebitNotes(debitNotes.map(dn => dn._id || dn.id!));
    } else {
      setSelectedDebitNotes([]);
    }
  };

  const handleSelectDebitNote = (id: string, checked: boolean) => {
    if (checked) {
      setSelectedDebitNotes(prev => [...prev, id]);
    } else {
      setSelectedDebitNotes(prev => prev.filter(dnId => dnId !== id));
    }
  };

  // Validation modal handlers
  const handleOpenValidationModal = (debitNote: DebitNote) => {
    setSelectedDebitNoteForValidation(debitNote);
  ...
  const handleCloseValidationModal = () => {
    ...
    setSelectedDebitNoteForValidation(null);
    setValidationResult(null);
    setIsValidating(false);
    setIsSubmitting(false);
  };

  const handleValidateDebitNote = async () => {
    if (!selectedDebitNoteForValidation) return;

    setIsValidating(true);
    setValidationResult(null);

    try {
      const debitNoteId = selectedDebitNoteForValidation._id || selectedDebitNoteForValidation.id!;
      const result = await DebitNoteService.validateDebitNote(debitNoteId);
  ...
  const handleSubmitToZatca = async () => {
    ...
    try {
      const debitNoteId = selectedDebitNoteForValidation._id || selectedDebitNoteForValidation.id!;
      const result = await DebitNoteService.sendDebitNote(debitNoteId);
      ...
      if (result.success) {
        ...
        setValidationResult({
          isValid: false,
          errors: [result.message || 'Failed to submit debit note to ZATCA'],
          warnings: []
        });
      }
    } catch (error) {
  ...
  // Get pending debit notes from selection
  const getPendingSelectedDebitNotes = () => {
    return debitNotes.filter(dn => {
      const debitNoteId = dn._id || dn.id;
      const isSelected = selectedDebitNotes.includes(debitNoteId!);
      const isPending = !dn.zatca?.status || dn.zatca?.status === 'pending';
      const isNotSent = dn.status !== 'sent' && dn.status !== 'paid';
      return isSelected && isPending && isNotSent;
    });
  };

  const pendingSelectedCount = getPendingSelectedDebitNotes().length;

  // Bulk validate
  const handleBulkValidate = async () => {
    const pendingDebitNotes = getPendingSelectedDebitNotes();

    if (pendingDebitNotes.length === 0) {
      showAlert('No pending debit notes selected for validation. Only debit notes with ZATCA status "Pending" can be validated.', 'warning');
      return;
    }
    ...
    setBulkValidationProgress({
      current: 0,
      total: pendingDebitNotes.length,
      results: []
    });
    ...
    for (let i = 0; i < pendingDebitNotes.length; i++) {
      const debitNote = pendingDebitNotes[i];
      const debitNoteId = debitNote._id || debitNote.id!;
      const debitNoteNumber = debitNote.debitNoteNumber || debitNote.invoiceNumber || debitNoteId;
  ...
  // Get deletable debit notes
  const getDeletableSelectedDebitNotes = () => {
    return debitNotes.filter(dn => {
      const debitNoteId = dn._id || dn.id;
      const isSelected = selectedDebitNotes.includes(debitNoteId!);
      const isDraft = dn.status === 'draft';
      const zatcaStatus = dn.zatca?.status;
      const isCleared = zatcaStatus === 'cleared' || zatcaStatus === 'reported';
      return isSelected && isDraft && !isCleared;
    });
  };

  const deletableSelectedCount = getDeletableSelectedDebitNotes().length;

  // Get downloadable debit notes
  const getSelectedDebitNotesForDownload = () => {
    return debitNotes.filter(dn => {
      const debitNoteId = dn._id || dn.id;
      return selectedDebitNotes.includes(debitNoteId!);
    });
  };

  const downloadSelectedCount = getSelectedDebitNotesForDownload().length;

  // Bulk delete handlers
  const handleBulkDeleteClick = () => {
    const deletableDebitNotes = getDeletableSelectedDebitNotes();

    if (deletableDebitNotes.length === 0) {
      showAlert('No deletable debit notes selected. Only draft debit notes that are not cleared by ZATCA can be deleted.', 'warning');
      return;
    }

    setDebitNotesToDelete(deletableDebitNotes);
  ...
  const handleConfirmBulkDelete = async () => {
    ...
    if (debitNotesToDelete.length === 0) return;

    setIsBulkDeleting(true);
    setBulkDeleteProgress({
      current: 0,
      total: debitNotesToDelete.length,
      results: []
    });
    ...
    for (let i = 0; i < debitNotesToDelete.length; i++) {
      const debitNote = debitNotesToDelete[i];
      const debitNoteId = debitNote._id || debitNote.id!;
      const debitNoteNumber = debitNote.debitNoteNumber || debitNote.invoiceNumber || debitNoteId;

      try {
        const success = await DebitNoteService.deleteDebitNote(debitNoteId);
        results.push({
          debitNoteId,
          debitNoteNumber,
          success,
  ...
  const handleBulkDownloadClick = () => {
    const selectedForDownload = getSelectedDebitNotesForDownload();

    if (selectedForDownload.length === 0) {
      showAlert('No debit notes selected for download.', 'warning');
      return;
    }

    setDebitNotesToDownload(selectedForDownload);
  ...
  const handleConfirmBulkDownload = async () => {
    ...
    if (debitNotesToDownload.length === 0) return;

    setIsBulkDownloading(true);
    setBulkDownloadProgress({
      current: 0,
      total: debitNotesToDownload.length,
      results: []
    });
    ...
    for (let i = 0; i < debitNotesToDownload.length; i++) {
      const debitNote = debitNotesToDownload[i];
      const debitNoteId = debitNote._id || debitNote.id!;
      const debitNoteNumber = debitNote.debitNoteNumber || debitNote.invoiceNumber || debitNoteId;

      try {
        const fullDebitNote = await DebitNoteService.getDebitNoteById(debitNoteId);

        if (fullDebitNote?.zatca?.pdfUrl) {
          const pdfData = fullDebitNote.zatca.pdfUrl;
          zip.file(`${debitNoteNumber}.pdf`, pdfData, { base64: true });
    ...
    if (successCount > 0) {
      try {
        ...
        saveAs(content, `debit-notes-${dateStr}.zip`);
      } catch (error) {
        console.error('Failed to generate ZIP:', error);
      }
    }

    setDebitNotesToDownload([]);
  };
  ...
  // Individual actions
  const handleSendDebitNote = async (id: string) => {
    if (confirm('Are you sure you want to send this debit note?')) {
      const result = await DebitNoteService.sendDebitNote(id);
      if (result.success) {
        loadDebitNotes();
        loadStats();
      }
  ...
  const handleDeleteDebitNote = async (id: string) => {
    if (confirm('Are you sure you want to delete this debit note? This action cannot be undone.')) {
      const success = await DebitNoteService.deleteDebitNote(id);
      if (success) {
        loadDebitNotes();
        loadStats();
      }
  ...
  const handleViewDebitNote = (id: string) => {
    router.push(`/dashboard/sales/debit-notes/view/${id}`);
  };

  const handleDownloadDebitNote = async (id: string) => {
    try {
      const debitNote = await DebitNoteService.getDebitNoteById(id);
      if (!debitNote) {
        showAlert('Debit note not found', 'error');
        return;
      }
      if (debitNote.zatca?.pdfUrl) {
        ...
        link.download = `debit-note-${debitNote.debitNoteNumber || debitNote.invoiceNumber || id}.pdf`;
      ...
      console.error('Error downloading debit note:', error);
      showAlert('Failed to download debit note PDF', 'error');
    }
  };
  ...
  const getZatcaStatusBadge = (zatca: DebitNote['zatca']) => {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage debit notes for additional charges</p>
        </div>
        <div className="flex items-center space-x-3">
          {permissions.canCreate && (
            <button
              onClick={() => router.push('/dashboard/sales/debit-notes/create')}
          ...
          {selectedDebitNotes.length > 0 && (
            <div className="flex items-center space-x-2 px-3 py-1.5 bg-gray-100 rounded-lg">
              <span className="text-sm text-gray-600 font-medium">
                {selectedDebitNotes.length} selected:
              </span>
              ...
              {permissions.canSend && (
                <button
                  ...
                  className="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  title={pendingSelectedCount === 0 ? 'No pending debit notes' : `Validate ${pendingSelectedCount} pending debit note(s)`}
                >
              ...
              {permissions.canDownload && (
                <button
                  ...
                  title={`Download ${downloadSelectedCount} debit note(s) as ZIP`}
                >
              ...
              {permissions.canDelete && (
                <button
                  ...
                  title={deletableSelectedCount === 0 ? 'No deletable debit notes' : `Delete ${deletableSelectedCount} draft debit note(s)`}
                >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {isStatsLoading ? '...' : stats.totalDebitNotes}
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {isStatsLoading ? '...' : stats.draftDebitNotes}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {isStatsLoading ? '...' : stats.paidDebitNotes}
              </p>
            </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <button
            onClick={() => { loadDebitNotes(); loadStats(); }}
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-12">
          <div className="text-center">
            ...
            <p className="text-gray-500 mb-6">Get started by creating your first debit note</p>
            {permissions.canCreate && (
              <Link
                href="/dashboard/sales/debit-notes/create"
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left">
                    <input
                      type="checkbox"
                      checked={selectedDebitNotes.length === debitNotes.length && debitNotes.length > 0}
                      onChange={(e) => handleSelectAll(e.target.checked)}
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Debit Note</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {debitNotes.map((debitNote) => {
                  const debitNoteId = debitNote._id || debitNote.id!;
                  const isPending = !debitNote.zatca?.status || debitNote.zatca?.status === 'pending';
                  const isDraft = debitNote.status === 'draft';

                  return (
                    <tr key={debitNoteId} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-4">
                        <input
                          type="checkbox"
                          checked={selectedDebitNotes.includes(debitNoteId)}
                          onChange={(e) => handleSelectDebitNote(debitNoteId, e.target.checked)}
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{debitNote.debitNoteNumber || debitNote.invoiceNumber}</div>
                          <div className="text-xs text-gray-500">{debitNote.items?.length || 0} items</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900">
                            {typeof debitNote.customerId === 'object' ? debitNote.customerId.customerName : 'N/A'}
                          </div>
                          <div className="text-xs text-gray-500">
                            {typeof debitNote.customerId === 'object' ? debitNote.customerId.contactInfo?.email || '' : ''}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {formatDate(debitNote.issueDate || debitNote.invoiceDate)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{getZatcaStatusBadge(debitNote.zatca)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end space-x-2">
                          <button onClick={() => handleViewDebitNote(debitNoteId)} className="text-gray-600 hover:text-gray-900" title="View">
                            <Eye className="h-4 w-4" />
                          </button>
                          {permissions.canEdit && isDraft && (
                            <Link href={`/dashboard/sales/debit-notes/edit/${debitNoteId}`} className="text-gray-600 hover:text-gray-900" title="Edit">
                              <Edit className="h-4 w-4" />
                            </Link>
                          )}
                          {permissions.canSend && isDraft && isPending && (
                            <button onClick={() => handleOpenValidationModal(debitNote)} className="text-blue-600 hover:text-blue-800" title="Validate & Send">
                              <ShieldCheck className="h-4 w-4" />
                            </button>
                          )}
                          {permissions.canSend && isDraft && !isPending && debitNote.zatca?.validationStatus === 'valid' && (
                            <button onClick={() => handleSendDebitNote(debitNoteId)} className="text-blue-600 hover:text-blue-800" title="Send">
                              <Send className="h-4 w-4" />
                            </button>
                          )}
                          {permissions.canDownload && (
                            <button onClick={() => handleDownloadDebitNote(debitNoteId)} className="text-gray-600 hover:text-gray-900" title="Download">
                              <Download className="h-4 w-4" />
                            </button>
                          )}
                          {permissions.canDelete && isDraft && !debitNote.zatca?.status?.match(/cleared|reported/) && (
                            <button onClick={() => handleDeleteDebitNote(debitNoteId)} className="text-red-600 hover:text-red-800" title="Delete">
      ...
      <ValidationModal
        ...
        debitNote={selectedDebitNoteForValidation}
        isValidating={isValidating}
        validationResult={validationResult}
        onValidate={handleValidateDebitNote}
        onSubmit={handleSubmitToZatca}
        isSubmitting={isSubmitting}
      />

      {/* Delete Confirmation Modal */}
      {showDeleteConfirmModal && debitNotesToDelete.length > 0 && (
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 bg-red-50 rounded-t-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{debitNotesToDelete.length} debit note(s) will be deleted</p>
                </div>
              </div>
              <div className="px-6 py-4">
                <p className="text-sm text-gray-600 mb-3">The following debit notes will be permanently deleted:</p>
                <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-100">
                  {debitNotesToDelete.map((dn) => (
                    <div key={dn._id || dn.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{dn.debitNoteNumber || dn.invoiceNumber}</p>
                        <p className="text-xs text-gray-500">
                          {typeof dn.customerId === 'object' ? dn.customerId.customerName : 'N/A'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-medium text-blue-600">{formatCurrency(dn.total, dn.currency)}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3 bg-gray-50 rounded-b-xl">
                ...
                <button onClick={handleConfirmBulkDelete} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors inline-flex items-center gap-2">
                  <Trash2 className="h-4 w-4" />
                  Delete {debitNotesToDelete.length} Debit Note(s)
                </button>
              </div>
            </div>
          </div>
      ...
      {showDownloadConfirmModal && debitNotesToDownload.length > 0 && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 bg-emerald-50 rounded-t-xl">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Download Debit Notes</h3>
                  <p className="text-sm text-gray-500">{debitNotesToDownload.length} debit note(s) will be downloaded as ZIP</p>
                </div>
              </div>
              <div className="px-6 py-4">
                <p className="text-sm text-gray-600 mb-3">The following debit notes will be included in the ZIP file:</p>
                <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-100">
                  {debitNotesToDownload.map((dn) => {
                    const hasPdf = dn.zatca?.pdfUrl || dn.zatca?.status === 'cleared' || dn.zatca?.status === 'reported';
                    return (
                      <div key={dn._id || dn.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{dn.debitNoteNumber || dn.invoiceNumber}</p>
                ...
                <p className="text-xs text-gray-500 mt-3">Only debit notes cleared by ZATCA will have PDF files in the ZIP</p>
              </div>
              <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3 bg-gray-50 rounded-b-xl">
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Validating {bulkValidationProgress.total} debit note(s)</p>
                </div>
              ...
              <div className="px-6 py-4">
                <div className="mb-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-2.5 rounded-full transition-all duration-300" style={{ width: `${(bulkValidationProgress.current / bulkValidationProgress.total) * 100}%` }} />
                  </div>
                </div>
                {bulkValidationProgress.current < bulkValidationProgress.total && (
                  <div className="flex items-center justify-center py-4">
                    <Loader2 className="h-6 w-6 text-blue-600 animate-spin mr-2" />
                    <span className="text-gray-600">Validating debit notes...</span>
                  </div>
                )}
                    ...
                    {bulkValidationProgress.results.map((result, index) => (
                      <div key={index} className={`flex items-start gap-3 p-3 rounded-lg ${result.success ? 'bg-green-50' : 'bg-red-50'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-medium ${result.success ? 'text-green-800' : 'text-red-800'}`}>{result.debitNoteNumber}</p>
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Deleting {bulkDeleteProgress.total} debit note(s)</p>
                </div>
              ...
              <div className="px-6 py-4">
                ...
                {bulkDeleteProgress.current < bulkDeleteProgress.total && (
                  <div className="flex items-center justify-center py-4">
                    ...
                    <span className="text-gray-600">Deleting debit notes...</span>
                  </div>
                )}
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Downloading {bulkDownloadProgress.total} debit note(s)</p>
                </div>
              ...
              <div className="px-6 py-4">
                ...
                {bulkDownloadProgress.current < bulkDownloadProgress.total && (
                  <div className="flex items-center justify-center py-4">
                    ...
                    <span className="text-gray-600">Downloading debit notes...</span>
                  </div>
                )}
                    ...
                    <button
                      ...
                      className={`relative inline-flex items-center px-4 py-2 text-sm font-semibold ${
                        pageNum === pagination.current
                          ? 'z-10 bg-gradient-to-r from-primary via-blue-600 to-indigo-700 text-white'
